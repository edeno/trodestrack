...
The `trodestrack filter` CLI command is **forward-filter-only** (no
...
streaming — there is no per-frame ingest loop.
...
exposed as a CLI today. See `uv run trodestrack filter --help` for the
batch CLI.
