...
def random_lgssm_args(
    key: Union[int, PRNGKey] = 0, num_timesteps: int = 15, state_dim: int = 4, emission_dim: int = 2
    ...
    *keys, subkey = jr.split(key, 9)
    ...
    params = ParamsLGSSM(
        initial=ParamsLGSSMInitial(mean=initial_mean, cov=initial_covariance),
...
def make_nlgssm_params(
    state_dim, emission_dim, dynamics_degree=1, emission_degree=1, key=jr.PRNGKey(0)
):
...
class SimpleNonlinearSSM(SSM):
    ...
    def initialize(self, key):
        ...
        params = dict(
            ...
            dynamics=dict(
                weights=jr.normal(
                    key1, (self.state_dim, self.state_dim * (self.dynamics_degree + 1))
                ),
                cov=jnp.eye(self.state_dim),
            ),
            emissions=dict(
                weights=jr.normal(
                    key2, (self.emission_dim, self.state_dim * (self.emission_degree + 1))
                ),
    ...
    def _make_inference_args(self, params):
        f = lambda state: jnp.sin(
            params["dynamics"]["weights"] @ to_poly(state, self.dynamics_degree)
        )
        h = lambda state: jnp.cos(
            params["emissions"]["weights"] @ to_poly(state, self.emission_degree)
        )
        return ParamsNLGSSM(
            initial_mean=params["initial"]["mean"],
            initial_covariance=params["initial"]["cov"],
