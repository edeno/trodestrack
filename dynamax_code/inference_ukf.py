...
def _compute_weights(n, alpha, beta, lamb):
    ...
    w_cov = jnp.concatenate(
        (jnp.array([lamb / (n + lamb) + (1 - alpha**2 + beta)]), jnp.ones(2 * n) * factor)
    )
    return w_mean, w_cov
...
def _predict(m, P, f, Q, lamb, w_mean, w_cov, u):
    ...
    P_pred = (
        jnp.tensordot(w_cov, _outer(sigmas_pred_prop - m_pred, sigmas_pred_prop - m_pred), axes=1)
        + Q
    )
...
def _condition_on(m, P, h, R, lamb, w_mean, w_cov, u, y):
    ...
    pred_cov = (
        jnp.tensordot(
            w_cov, _outer(sigmas_cond_prop - pred_mean, sigmas_cond_prop - pred_mean), axes=1
        )
        + R
    )
