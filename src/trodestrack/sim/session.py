...
def _generate_ground_truth_trajectory(config: SimConfig) -> Dict[str, np.ndarray]:
    ...
    positions = np.zeros((n_samples, 2))  # [x, y] in cm
    velocities = np.zeros((n_samples, 2))  # [vx, vy] in cm/s
    ...
    headings[0] = np.random.uniform(0, 2 * np.pi)

    # Initial velocity (random direction, moderate speed)
    initial_speed = np.random.uniform(5, 20)  # cm/s
    velocities[0] = [
    ...
    for i in range(1, n_samples):
        ...
        if np.random.random() < config.trajectory.turn_probability:
            # Apply random turn
            turn_angle = np.random.normal(0, 0.5)  # radians
            ...
            accel_magnitude = np.random.normal(0, config.trajectory.max_acceleration / 10)
            accel_direction = np.random.uniform(0, 2 * np.pi)

            accel = accel_magnitude * np.array([
                np.cos(accel_direction),
                np.sin(accel_direction)
            ])
        ...
        if np.linalg.norm(velocities[i]) > 1e-6:  # Avoid division by zero
