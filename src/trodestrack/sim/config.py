...
class TrajectoryConfig(BaseModel):
    ...
    turn_probability: float = Field(
        ...
        ge=0,
        le=1,
        description="Probability of direction change per frame",
    )
...
class IMUConfig(BaseModel):
    ...
    accel_noise_std: float = Field(
        default=0.05, ge=0, description="Accelerometer noise standard deviation (g)"
    )
    gyro_noise_std: float = Field(
        default=0.02, ge=0, description="Gyroscope noise standard deviation (deg/s)"
    )
    accel_bias_std: float = Field(
        default=0.01, ge=0, description="Accelerometer bias standard deviation (g)"
    )
    gyro_bias_std: float = Field(
        default=0.005, ge=0, description="Gyroscope bias standard deviation (deg/s)"
    )
    bias_drift_std: float = Field(
        default=0.0005, ge=0, description="Bias drift standard deviation per second"
    )
    misalignment_deg: float = Field(
        default=1.0, ge=0, le=45, description="IMU misalignment in degrees"
    )
...
class VideoConfig(BaseModel):
    ...
    position_noise_std: float = Field(
        default=1.5, ge=0, description="Position noise standard deviation (pixels)"
    )
    confidence_min: float = Field(
        default=0.05, ge=0, le=1, description="Minimum detection confidence"
    )
    confidence_max: float = Field(
        default=0.98, ge=0, le=1, description="Maximum detection confidence"
    )
    occlusion_probability: float = Field(
        default=0.02, ge=0, le=1, description="Probability of occlusion per frame"
    )
    occlusion_duration_mean: float = Field(
        default=0.5, gt=0, description="Mean occlusion duration in seconds"
    )
    led_swap_probability: float = Field(
        default=0.005, ge=0, le=1, description="Probability of LED swap per frame"
    )
    dropout_probability: float = Field(
        default=0.01, ge=0, le=1, description="Probability of frame dropout"
    )
...
class LEDConfig(BaseModel):
    ...
    front_back_distance: float = Field(
        default=20.0, gt=0, description="Distance between front and back LEDs (pixels)"
    )
    swap_detection_threshold: float = Field(
        default=0.9, ge=0, le=1, description="Threshold for detecting LED swaps"
    )
