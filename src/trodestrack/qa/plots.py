...
def plot_trajectory_comparison(
    ...
    if occlusion_mask is not None:
        ...
        if len(occlusion_indices) > 0:
            ax_traj.scatter(est_pos[occlusion_indices, 0], est_pos[occlusion_indices, 1],
                          c='orange', s=20, alpha=0.6, label='Occluded', zorder=5)

    # Mark start and end points
    ax_traj.scatter(gt_pos[0, 0], gt_pos[0, 1], c='green', s=100, marker='o',
                   label='Start', zorder=6, edgecolors='black')
    ax_traj.scatter(gt_pos[-1, 0], gt_pos[-1, 1], c='red', s=100, marker='s',
                   label='End', zorder=6, edgecolors='black')
    ...
    # Highlight occlusion periods
    if occlusion_mask is not None:
        ax_error.fill_between(time_axis, 0, position_error,
                            where=occlusion_mask, alpha=0.3, color='orange',
                            label='Occluded')

    # PRD threshold line
    ax_error.axhline(y=2.0, color='red', linestyle='--', alpha=0.7,
                    label='PRD Threshold (2 cm)')
...
def plot_velocity_and_heading(
    ...
    if occlusion_mask is not None:
        axes[0, 0].fill_between(time_axis, 0, np.max([gt_speed.max(), est_speed.max()]),
                               where=occlusion_mask, alpha=0.2, color='orange')
    ...
    axes[0, 1].axhline(y=10.0, color='red', linestyle='--', alpha=0.7,
                      label='PRD Threshold (10 cm/s)')

    if occlusion_mask is not None:
        axes[0, 1].fill_between(time_axis, 0, vel_error,
                               where=occlusion_mask, alpha=0.3, color='orange')
    ...
    # Heading comparison
    axes[1, 0].plot(time_axis, np.degrees(gt_heading), 'k-', linewidth=2,
                   label='Ground Truth', alpha=0.8)
    axes[1, 0].plot(time_axis, np.degrees(est_heading), 'r-', linewidth=1.5,
                   label='Estimated', alpha=0.7)

    if occlusion_mask is not None:
        y_min, y_max = axes[1, 0].get_ylim()
        axes[1, 0].fill_between(time_axis, y_min, y_max,
                               where=occlusion_mask, alpha=0.2, color='orange')
    ...
    axes[1, 1].axhline(y=7.0, color='red', linestyle='--', alpha=0.7,
                      label='PRD Threshold (7°)')

    if occlusion_mask is not None:
        axes[1, 1].fill_between(time_axis, 0, heading_error,
                               where=occlusion_mask, alpha=0.3, color='orange')
...
def plot_nees_analysis(
    ...
    axes[0, 0].axhline(y=expected_nees, color='red', linestyle='--', linewidth=2,
                      label=f'Expected ({expected_nees:.0f})')
    axes[0, 0].axhline(y=full_nees["nees_mean"], color='green', linestyle='-',
                      alpha=0.7, label=f'Mean ({full_nees["nees_mean"]:.1f})')

    # 95% confidence interval for chi-squared with 8 DOF
    chi2_lower = 2.18  # 2.5th percentile
    chi2_upper = 15.51  # 97.5th percentile
    axes[0, 0].axhline(y=chi2_lower, color='orange', linestyle=':', alpha=0.7,
                      label='95% CI')
    ...
    axes[0, 1].plot(time_axis, pos_nees_values, 'g-', linewidth=1.5, alpha=0.7, label='Position NEES')
    axes[0, 1].axhline(y=pos_expected, color='red', linestyle='--', linewidth=2,
                      label=f'Expected ({pos_expected:.0f})')
    axes[0, 1].axhline(y=pos_nees["position_nees_mean"], color='blue', linestyle='-',
                      alpha=0.7, label=f'Mean ({pos_nees["position_nees_mean"]:.1f})')
    ...
    axes[0, 1].axhline(y=chi2_2dof_lower, color='orange', linestyle=':', alpha=0.7,
                      label='95% CI')
    ...
    axes[1, 0].hist(nees_values, bins=30, density=True, alpha=0.7, color='blue',
                   label='Observed')
    ...
    axes[1, 0].axvline(x=expected_nees, color='red', linestyle='--', alpha=0.7,
                      label=f'Expected ({expected_nees:.0f})')
    axes[1, 0].axvline(x=full_nees["nees_mean"], color='green', linestyle='-',
                      alpha=0.7, label=f'Mean ({full_nees["nees_mean"]:.1f})')
    ...
    axes[1, 1].hist(pos_nees_values, bins=30, density=True, alpha=0.7, color='green',
                   label='Observed')
    ...
    axes[1, 1].axvline(x=pos_expected, color='red', linestyle='--', alpha=0.7,
                      label=f'Expected ({pos_expected:.0f})')
    axes[1, 1].axvline(x=pos_nees["position_nees_mean"], color='blue', linestyle='-',
                      alpha=0.7, label=f'Mean ({pos_nees["position_nees_mean"]:.1f})')
...
def plot_bias_traces(
    ...
    if ground_truth_states is not None:
        ...
        axes[0].plot(time_axis, gt_bias_gz, 'k--', linewidth=2,
    ...
    if ground_truth_states is not None:
        ...
        axes[1].plot(time_axis, gt_bias_ax, 'k--', linewidth=2,
                    label='GT Accel X', alpha=0.7)
        axes[1].plot(time_axis, gt_bias_ay, 'k:', linewidth=2,
                    label='GT Accel Y', alpha=0.7)
...
def plot_measurement_residuals(
    ...
    if 'position' in residuals:
        ...
        if pos_residuals.ndim == 2 and pos_residuals.shape[1] >= 2:
            ...
            axes[0, 0].plot(time_axis, pos_residuals[:, 0], 'r-', linewidth=1.5,
                           label='X residual', alpha=0.8)
            axes[0, 1].plot(time_axis, pos_residuals[:, 1], 'g-', linewidth=1.5,
                           label='Y residual', alpha=0.8)
            ...
            if measurement_validity and 'position' in measurement_validity:
                ...
                if np.any(invalid_mask):
                    axes[0, 0].scatter(time_axis[invalid_mask],
                                     pos_residuals[invalid_mask, 0],
                                     c='red', s=20, alpha=0.5, label='Invalid')
                    axes[0, 1].scatter(time_axis[invalid_mask],
                                     pos_residuals[invalid_mask, 1],
                                     c='red', s=20, alpha=0.5, label='Invalid')
        else:
            # Single residual magnitude
            axes[0, 0].plot(time_axis, pos_residuals, 'b-', linewidth=1.5,
                           label='Position residual', alpha=0.8)
    ...
    if 'heading' in residuals:
        ...
        axes[1, 0].plot(time_axis, heading_residuals_deg, 'purple', linewidth=1.5,
                       label='Heading residual', alpha=0.8)

        # Mark invalid measurements
        if measurement_validity and 'heading' in measurement_validity:
            invalid_mask = ~measurement_validity['heading']
            if np.any(invalid_mask):
                axes[1, 0].scatter(time_axis[invalid_mask],
    ...
    if 'position' in residuals:
        ...
        axes[1, 1].plot(time_axis, residual_mag, 'blue', linewidth=1.5,
                       label='Position magnitude', alpha=0.8)
    ...
    if 'heading' in residuals:
        ...
        axes[1, 1].plot(time_axis, heading_mag_scaled, 'purple', linewidth=1.5,
                       label='Heading magnitude (scaled)', alpha=0.8)
