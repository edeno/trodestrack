...
class IMUConfig(BaseModel):
    ...
    downsampling_rate: float = Field(
        default=1000.0, description="Target IMU rate for processing (Hz)", gt=0.0
    )
    alignment_correction: Optional[List[List[float]]] = Field(
        default=None, description="3x3 rotation matrix for IMU-to-body frame alignment"
    )
...
class LEDConfig(BaseModel):
    ...
    front_back_distance_cm: float = Field(
        description="Expected distance between front and back LEDs (cm)", gt=0.0
    )
    ...
    confidence_threshold: float = Field(
        default=0.5, description="Minimum confidence for LED detections", ge=0.0, le=1.0
    )
...
class MappingConfig(BaseModel):
    ...
    type: str = Field(description="Mapping type", pattern="^(homography|ruler_scale)$")
    homography_matrix: Optional[List[List[float]]] = Field(
        default=None, description="3x3 homography matrix for pixel->cm transformation"
    )
    pixel_per_cm: Optional[float] = Field(
        default=None, description="Ruler-based scale (pixels per cm)", gt=0.0
    )
    arena_bounds: Optional[Tuple[float, float, float, float]] = Field(
        default=None, description="Arena bounds in cm (x_min, y_min, x_max, y_max)"
    )
...
class FilterConfig(BaseModel):
    ...
    measurement_noise: dict = Field(
        default_factory=lambda: {"position": 1.0, "heading": 0.1},
    ...
    velocity_damping: float = Field(
        default=0.0, description="Velocity damping coefficient (λ in dynamics)", ge=0.0
    )
...
class SessionConfig(BaseModel):
    ...
    video_file: Optional[Path] = Field(
        default=None, description="Path to video detection file (Trodes LED or DLC)"
    )
    imu_file: Optional[Path] = Field(
        default=None, description="Path to IMU data file (SpikeGadgets format)"
    )

    # Frame rate and timing
    video_fps: float = Field(default=30.0, description="Video frame rate (Hz)", gt=0.0)

    # Configuration components
    mapping: MappingConfig = Field(description="Coordinate mapping configuration")
    filter: FilterConfig = Field(
        default_factory=FilterConfig, description="Kalman filter configuration"
    )
