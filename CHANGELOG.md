...
Six-phase post-0.2.0 review pass: critical silent-failure fixes, comprehensive test coverage, scaffolding cleanup, type hardening, CLI UX overhaul, and filter polish. Net delta vs 0.2.0: ~+3000 src LOC, ~+3500 test LOC, test count 887 → 967.

### Changed (breaking)

- **CLI**: The `online` CLI subcommand has been renamed to `filter`. Update scripts: `trodestrack online ...` → `trodestrack filter ...`. The semantics are unchanged — the new name accurately reflects that the command runs a forward-only batch filter (not a streaming/real-time ingest loop). No alias.
...
- **`FilterCoreConfig(state_mode="vision_only", enable_zupt=True)`** now raises `ValueError` instead of silently disabling ZUPT. Set `enable_zupt=False` explicitly when constructing the config directly. The YAML config loader and the CLI `--state-mode vision_only` path auto-disable ZUPT (mirroring the existing `use_mahalanobis_gating` shim) so YAML / CLI users are unaffected.
...
- **QA plots & report**: public function parameters renamed `prd_threshold_m` → `target_threshold_m` (`qa.plots.plot_position_error` / `plot_velocity_error`) and `prd_threshold_deg` → `target_threshold_deg` (`plot_heading_error`). Module-level constants `qa.report.PRD_*` → `TARGET_*` (not exported in `__all__`). User-visible plot legend labels change from `"PRD: ±10 cm/s"` to `"target: ±10 cm/s"`.
- **`build_quaternion_transition_jacobian`** now requires `u_imu` (previously optional with a documented inconsistency warning); every real call site passed it.
- **`update_zupt`** now requires an explicit `layout: StateLayout` keyword parameter, removing the dimension-based `LAYOUT_REGISTRY` lookup. Two registered 16D layouts no longer collide silently; future layouts sharing a dimension can't miswire.
...
- **Progress messages** during long filter / smoother runs: total wall-clock time + frames/second on completion, with a note that the first run includes JIT compilation cost.
- **QA report summary page** now leads with a `RESULT: PASS` or `RESULT: FAIL (failing metrics)` verdict line based on project acceptance targets.
...
- **`--state-mode`** argparse `choices=` now uses `STATE_MODES`; help text lists every registered mode automatically.
- **Test coverage** (+80 tests):
  - `cli.report` coverage 0% → 94% (5 behavioral + 15 parametrized validation tests + 2 `--from-run` bridge tests)
  - 5 new 3D EKF analytic tests (stationary pitch/roll, yaw-only bias convergence, 5s dropout drift, NEES, perfect-input idempotence)
  - 5 new UKF 3D rejection-contract tests (documenting the `NotImplementedError` contract for future analytic-bound conversion)
  - Gimbal-lock / pitch-singularity tests for `estimate_orientation`
  - 2 real-vision-EKF safety-check tests (no mocks)
  - CLI smoothed-output finiteness + PSD tests
  - Sensor unit tests for `heading_pseudo` and `camera_position` (Jacobian vs `jacfwd` parity, innovation wrap, gate behavior)
  - UKF 5s dropout drift parity (≤ 2× EKF)
  - `simulate_3d_session` deterministic 3D simulator in `tests/filters/conftest.py`
- **`pypdf`** added as a `[dev]` dependency so PDF-content tests can assert on text inside compressed page streams.
...
- `io.session._index_or_time_column` now raises `ValueError` when the input DataFrame is missing the `time` column. The previous fallback silently substituted sample numbers (df.index) for seconds, producing dt values off by the sampling rate. Accepts a `time`-named DataFrame index for compatibility with Trodes-style parquets.
...
- `cli.utils.load_data_file` error messages include the exception class name (e.g. `ValueError`, `IOError`).
- `io.session._validate_time_vector` error messages now include the offending index, the timestamp values at that step, and common-cause recovery hints.
...
### Changed (internal — no behavior change, bit-exact verified)

- 3D IEKF (`extended_kalman_filter_3d`) now uses `lax.scan` for the iterated-update loop, matching the 2D path. JIT compilation no longer unrolls the Python `for` linearly with `config.num_iter`; 0.000e+00 max delta on the 3D analytic parity oracle.
- `Camera3DPositionModel.geometric_jacobian` is now an analytic Jacobian (closed-form `∂R(q)/∂q` for scalar-first quaternions, chained through `q/||q||` normalization) instead of `jacfwd(self.predict)`. Agrees with `jacfwd` to float32 XLA tolerance on 50 random unit-norm states.
- `process_noise.assemble_Q` per-component multipliers are now vectorized via outer-product scaling instead of 12+ per-index `.at[idx, idx].mul()` calls.
- `_outer_product_batch` in `models/ukf.py` simplified from `vmap(atleast_2d.T @ atleast_2d)` to `vmap(jnp.outer)`.
- Dead `add_more_jitter` fallback removed from `gaussian_log_likelihood`. The leading adaptive jitter plus upstream `symmetrize` is sufficient on healthy inputs; an indefinite matrix now surfaces via `slogdet`'s sign field rather than silently double-jittering.
- `extended_kalman_filter` and `unscented_kalman_filter` use a `_with_led_distance` helper that shallow-clones the config without re-running `FilterCoreConfig.__post_init__`.
...
- Swept ~140 trivial WHAT-comments from `viz/components.py`, `models/ukf.py`, `models/ekf.py`, and `sim/rat_imu.py`.
- Renamed PRD-prefixed test files and internal test constants to `target_*`.

### Removed

- README "Project Status" section consolidated into the CHANGELOG. Benchmark numbers (~38× realtime / ~0.41 ms per frame on M-series Mac CPU) now live in one place; the README on-ramp links to the CHANGELOG.
...
[0.3.0]: https://github.com/edeno/trodestrack/compare/v0.2.0...v0.3.0
